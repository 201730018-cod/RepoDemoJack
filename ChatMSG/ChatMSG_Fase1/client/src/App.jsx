  ...
  return (
    <div style={styles.loginWrapper}>
      <div style={styles.loginCard}>
        ...
        <p style={styles.loginSub}>Fase 1 — sin persistencia</p>
        <form onSubmit={handleSubmit} style={styles.loginForm}>
          <input
            ...
            autoFocus
          />
          <button style={styles.btnPrimary} type="submit" disabled={!username.trim()}>
            Entrar al chat
          </button>
        </form>
      </div>
...
// ─── Componente Sidebar ───────────────────────────────────────────────────────
function Sidebar({ rooms, currentRoom, onJoinRoom, onCreateRoom, username }) {
...
export default function App() {
  ...
  useEffect(() => {
    socket.on("registered", ({ userId: id, rooms: r }) => {
      setUserId(id);
      setRooms(r);
      // Unirse a "general" automáticamente
...
const styles = {
  ...
  loginSub: { color: "var(--text-muted)", fontSize: 13, marginBottom: 28 },
